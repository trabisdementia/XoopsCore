...
$xoops->header('admin:system/system-users.tpl');
...
switch ($op) {
    ...
    default:
        ...
        if (isset($_REQUEST['complet_search'])) {
            ...
            // Assign Breadcrumb menu
            $admin_page = new \Xoops\Module\Admin();
            $admin_page->addBreadcrumbLink(SystemLocale::USERS_MANAGEMENT);

            $admin_page->addTool(
                XoopsLocale::A_SYNCHRONIZE,
                'xicon-update',
                array(
                    'href'  => 'users.php?op=users_synchronize&amp;status=2'
                )
            );

            $admin_page->addTool(
                SystemLocale::ADD_USER,
                'xicon-user-add',
                array(
                    'href'  => 'users.php?op=users_add'
                )
            );
            ...
            $group_handler = $xoops->getHandlerGroup();
            $group_arr = $group_handler->getObjects();
            $xoops->tpl()->assign('group_arr', $group_handler->getObjects());
            $xoops->tpl()->assign('user_type', $user_type);
        ...
        $xoops->tpl()->assign('systemLang', [
            'submit' => __('Submit', 'system'),
            'search' => __('Search', 'system'),
            'advancedSearch' => __('Advanced Search', 'system'),
            'searchUser' => __('Search User', 'system'),
            'allGroups' => __('All Groups', 'system'),
